...
variable "hub_bandwidth" {
  type = map(map(object({
    hubdscgid     = optional(string)
    leafdscgid    = optional(string)
    hubdscidlist  = optional(list(string))
    leafdscidlist = optional(list(string))
    direction     = optional(string) // possible values: bidi, us, ds
  })))
